...
public class FinalFractalImageGenerator {
    ...
    private Pixel[][] generateImage() {
        ...
        RawFractalImageGenerator rawFractalImageGenerator =
            new RawFractalImageGenerator(synchronizedPixelMap, affineFunctionParameters,
        ...
        if (userCommand.isParallelOn()) {
            pixels = rawFractalImageGenerator.parallelGenerate();
        } else {
            pixels = rawFractalImageGenerator.generate();
        }
